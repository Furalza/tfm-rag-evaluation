"""
Evaluate All RAG Systems - 20 Questions
========================================
Run from: ~/rag_benchmark/
Command : python evaluate_all.py

Evaluates whichever answer files exist - skips missing ones.
Output  : ~/rag_benchmark/all_evaluations.json
"""
...
# Full rubric from thesis Appendix
...
BASE = os.path.expanduser("~/rag_benchmark")

FILES = {
    "Baseline RAG": os.path.join(BASE, "baseline_answers.json"),
    "GraphRAG":     os.path.join(BASE, "graphrag_answers.json"),
    "LightRAG":     os.path.join(BASE, "lightrag_answers.json"),
    "Hybrid RAG":   os.path.join(BASE, "hybrid_results.json"),
    "Blocks-DB":    os.path.join(BASE, "blocks_rag_results.json"),
}
...
# Save
out = os.path.join(BASE, "all_evaluations.json")
with open(out, "w", encoding="utf-8") as f:
    json.dump(all_results, f, indent=2, ensure_ascii=False)

print(f"\n{'='*55}")
print(f"DONE — ALL SYSTEMS")
print(f"{'='*55}")
